...
class InterPackageRelate(unittest.TestCase):
    def test(self):
        ...
        local_lopkg = LocalPackage(root=lofs.rootdirectory(), setups=[CSetupFactory(short_libnames=False,
                                                                                    use_libtool=False)])
        local_lopkg.enlarge(external_nodes=[])
        installed_lopkg = local_lopkg.install()

        hifs = dirhier.packageroot(name='hi', version='1.2.3')
        hifs.rootdirectory().add(name='hi.c', entry=File())
        local_hipkg = LocalPackage(root=hifs.rootdirectory(), setups=[CSetupFactory()])
        local_hipkg.enlarge(external_nodes=installed_lopkg.nodes())

        lo_h_builder = find.find_entrybuilder(root=local_hipkg.rootbuilder(), path=['lo.h'])
        lo_c_builder = find.find_entrybuilder(root=local_hipkg.rootbuilder(), path=['lo.c'])
        liblo_builder = None
        ...
        self.failUnless(len(lo_c_builder.native_installed_modules_used()) == 1)
